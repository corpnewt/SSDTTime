...
""".replace("[[device]]",x["device"]).replace("[[parent]]",x["parent"]).replace("[[address]]",x.get("address","Name (_ADR, Zero)  // _ADR: Address")).replace("[[new_device]]",x["rename"])
    ...
    def main(self):
        ...
        print("8. GPI0          - Enables GPI0 _STA to enable trackpad with VoodooI2C")

        if sys.platform.startswith("linux") or sys.platform == "win32":
            print("9. Dump DSDT     - Automatically dump the system DSDT")
        print("")
        ...
        if menu == "1":
            ...
            self.ssdt_gpi0()
        elif menu == "9" and (sys.platform.startswith("linux") or sys.platform == "win32"):
